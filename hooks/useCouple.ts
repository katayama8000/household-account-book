...
export const useCouple = () => {
  const fetchCoupleIdByUserId = async (user_id: Couple["user1_id"] | Couple["user2_id"]) => {
    ...
    const user1 = await supabase.from(couples_table).select("id").eq("user1_id", user_id).single();
    const user2 = await supabase.from(couples_table).select("id").eq("user2_id", user_id).single();
